...
```{r}
model <- lm(Wins ~ `Yards Gained` + `Points Scored` + `Giveaways` , data = teams)
summary(model)
```
...
This model is kind of useful. Based on the variables, about 62% of the variation in wins meaning it is decent. Also, the F-statistic is statistically significant, showing the model works as a whole.
However, not all variables are helpful. Points Scored is clearly significant, but Yards Gained is not significant. So, some of the model is missing some important factors that affect wins.
...
The number of points scored has the biggest impact of the number of wins. There is not a variable that does not impact the number of wins at all. However, the number of yards gained is not impacted by the number of takeaways and vice-versa. 
...
This model can be used to cover a team or game because it can help find correlations in the data to help the team cover and win a game. For example, you could point out that scoring more points is strongly tied to winning, so if a team wins, you might highlight how their offense performed. On the flip side, if they lose, it can be mentioned that they did not gain a lot of yards ultimately resulting in their downfall. 
