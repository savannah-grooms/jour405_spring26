...
wh_paid |>
  summarize(
    avg_whsalary = mean(SALARY),
    median_whsalary = median(SALARY),
    sd_whsalary = sd(SALARY)
  )
...
wh_paid |> 
  ...
  geom_vline(xintercept = mean(wh_paid$SALARY), color = "red", linetype = "dashed", size = 1) 
```

**Describe the shape of the distribution. Is it roughly normal? Skewed? Where do most salaries cluster?**

The distribution is skewed to the right with most of the salaries clustered from $60,000 to $125,000. Most people are paid around 60,000 which is where the mode is situated. 
...
To my surprise, the data follows the empirical rule a lot closer than I thought. The actual percent for within 1 SD is 65.9, the actual percent for within 2 SD is 91.2 compared to 95% and the actual percent for 3 SD is 100%. This suggests that the salary distribution, while not perfectly normal, aligns reasonably well with the patterns predicted by the empirical rule. 
...
There is about an 18% chance that a White House employee earns more than $150,000. The normal approximation is about 17% so the normal model underestimates the probability slightly (about 1.3%). 
...
About 33% of people make this "middle range" salary in the White House. The normal model overestimates the data at 37%. Therefore, the data is more skewed and most salaries are either higher or lower than the middle range salaries. 
...
The reality differs the most from the below $60,000 range. The difference is -38.3% lower than what the normal model predicts.This means that the salary distribution for the White House employees are not symmetric like a normal curve but more skewed. Most people make at least $60,000 at the White House due to the cost of living in DC. 
...
White House Pay Lags Behind D.C Living Costs for Most Staff

Roughly 38% of White House employees earn salaries in the middle-pay range, from $60,000 to $90,0000. A level that may not stretch far in Washington, DC, where housing and expenses is ranked among the highest in the county, raising questions about how comfortably many staffers can afford to live near the nation's captial. 

The way I would take this story is I would compare the likelihood of earning a middle-range salary at the White House to similar policy jobs in other federal agencies. I would interview workers and then comparing these salary probabilities to the share of income typically spent on housing in D.C.
